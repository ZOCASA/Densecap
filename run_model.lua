...
cmd:option('-skip_images', 0, 'to skip images from directory')
...
function get_input_images(opt)
  ...
  elseif opt.input_dir ~= '' then
    -- counter to count no of images seen
    local counter = 0
    -- skip is one more then the option set
    local skip = opt.skip_images + 1
    -- iterate all files in input directory and add them to work
    for fn in paths.files(opt.input_dir) do
      if string.sub(fn, 1, 1) ~= '.' and counter % skip == 0 then
